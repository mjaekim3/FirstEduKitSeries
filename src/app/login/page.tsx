...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const moveTowards = (nextX: number, nextY: number, speed: number, elapsed: number) => {
      ...
      if (dist <= step) { x = nextX; y = nextY; draw(); return true }
      x += (dx / dist) * step
      y += (dy / dist) * step
    ...
    if (!window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) {
      ...
      const roam = (time: number) => {
        ...
        if (mode === "walk" && moveTowards(targetX, targetY, 75, elapsed)) {
          ...
          birdY = birdBaseY; chaseStart = time
          setMode("chase")
        } else if (mode === "chase") {
          ...
          const { maxX, maxY } = bounds()
          moveTowards(
            Math.max(0, Math.min(maxX, birdX - cat.offsetWidth / 2)),
            Math.max(0, Math.min(maxY, birdY + 20)),
            135, elapsed
          )
    ...
    const onResize = () => {
      ...
      targetX = Math.min(targetX, maxX); targetY = Math.min(targetY, maxY)
      draw()
    }
  ...
  return (
    <div className="login-scene min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-[radial-gradient(circle_at_top,#fafcf8_0%,#e8f0e9_60%,#dce9df_100%)]">
      <div ref={catRef} aria-hidden="true" className="login-cat pointer-events-none fixed left-0 top-0 z-0 h-40 w-[120px] opacity-0 sm:h-[200px] sm:w-[150px]">
